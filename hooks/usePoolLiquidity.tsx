...
export const getPoolLiquidity = async ({ poolId, tokenAddress, decimals }) => {
  const { liquidity } = await getMultiplePoolsLiquidity({
    pools: poolId !== undefined && tokenAddress.length > 0 ? [{ pool_id: poolId, token_address: tokenAddress, decimals }] : undefined,
  })
...
export const getMultiplePoolsLiquidity = async ({
  ...
  try {
    ...
    const balInfos = await Promise.all(
      pools.map((p) => {
        // @ts-ignore
        return hera.get_pool_shares({ account_id: account.accountId, pool_id: Number(p.pool_id) })
      })
    )
    ...
    const liquidity: LiquidityInfoType[] = pools.map((p, index) => {
      const decimals = p.token_address.map((token) => unsafelyGetTokenInfoFromAddress(token).decimals)
      const reserve: [number, number] = poolInfos[index].amounts.map((a, index2) => convertMicroDenomToDenom(a, decimals[index2]))
