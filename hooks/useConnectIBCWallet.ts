...
export const useConnectIBCWallet = (
  ...
  const mutation = useMutation(async () => {
    if (window && !window?.keplr) {
      alert('Please install Near extension and refresh the page.')
      return
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener('near_keystorechange', reconnectWallet)
    return () => {
      window.removeEventListener('near_keystorechange', reconnectWallet)
    }
